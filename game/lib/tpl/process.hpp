...
class Process {
  ...
  public:
    /// Note on Windows: it seems not possible to specify which pipes to
    /// redirect. Thus, at the moment, if read_stdout==nullptr,
    /// read_stderr==nullptr and open_stdin==false, the stdout, stderr and stdin
    /// are sent to the parent process instead.
    Process(
  ...
  private:
    Data data;
    bool closed;
    int exit_status_cache = -1; // cached result once the handle has been closed
