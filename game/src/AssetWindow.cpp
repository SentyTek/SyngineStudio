...
bool AssetWindow::ContainsInsensitive(const std::string& text,
...
void AssetWindow::BuildFileTree(scl::path   projectPath,
    ...
    for (const auto& [vfsFile, bundle, compressedSize, originalSize] :
        ...
        const bool isPackagedShader =
            (vfsFileStr.size() >= 9 &&
             vfsFileStr.compare(vfsFileStr.size() - 9, 9, ".vert.bin") == 0) ||
            (vfsFileStr.size() >= 9 &&
             vfsFileStr.compare(vfsFileStr.size() - 9, 9, ".frag.bin") == 0);
        ...
        const auto findOnDisk = [&diskSearchName,
            ...
            return std::find_if(
                ...
                [&diskSearchName, isPackagedShader](const auto& diskFile) {
                    ...
                    const std::string diskFileName =
                        diskPath.substr(diskFileNameOffset == std::string::npos
                                            ? 0
                    ...
                    return suffixOffset != std::string::npos &&
                           (suffixOffset == 0 ||
                            ...
                            diskPath[suffixOffset - 1] == '\\') &&
                           (isPackagedShader
                                ? suffixOffset + suffix.size() < diskPath.size()
...

        {
            AssetType   type = vfsFile.endswith(".glb")   ? AssetType::MODEL
                               ...
                               : vfsFile.endswith(".png") ? AssetType::TEXTURE
                                                          : AssetType::UNKNOWN;
            std::string stem =
                vfsFileStr.substr(vfsFileStr.find_last_of("/\\") + 1);
            scl::path diskFile =
                projectFile != projectAssets.end() ? *projectFile : *engineFile;
            ...
            if (type == AssetType::TEXTURE) {
                ...
                thumbnail = IconManager::GetIcon("component/mesh");
            } else if (type == AssetType::SHADER) {
                thumbnail = IconManager::GetIcon("assets/shader");
            }
...
// Only generated thumbnails are owned by the tree. Icon thumbnails are shared
// with IconManager and must remain valid across tree rebuilds.
void AssetWindow::_DestroyThumbnails(AssetDirectory* directory) {
    if (!directory) {
        return;
    }
    for (auto& asset : directory->assets) {
        if (asset.type == AssetType::TEXTURE &&
            bgfx::isValid(asset.thumbnail)) {
            bgfx::destroy(asset.thumbnail);
            asset.thumbnail = BGFX_INVALID_HANDLE;
        }
    }
    for (auto& child : directory->children) {
        _DestroyThumbnails(child.get());
    }
}

void AssetWindow::RebuildTree() {
    m_selectedAsset = nullptr;
    _DestroyThumbnails(g_rootDirectory.get());
    g_rootDirectory = std::make_unique<AssetDirectory>(AssetDirectory{
        .name = "Assets", .children = {}, .assets = {}, .parent = nullptr });
    BuildFileTree(m_projectRoot, m_projectName);
}

void AssetWindow::Shutdown() {
    m_selectedAsset = nullptr;
    _DestroyThumbnails(g_rootDirectory.get());
...
std::string
AssetWindow::GetDirectoryAbsolutePath(const AssetDirectory* directory) {
...
void AssetWindow::RequestDelete(const std::string& absolutePath,
                                bool               isDirectory,
...
void AssetWindow::RequestDeleteConfirmation(const std::string& absolutePath,
                                            bool               isDirectory,
                                            const std::string& label) {
    if (absolutePath.empty()) {
        return;
...
void AssetWindow::DrawDeleteConfirmationPopup() {
...
void AssetWindow::ProcessPendingDeletion() {
...
void AssetWindow::ShowInFileBrowser(const std::string& absolutePath) {
    if (absolutePath.empty()) {
        return;
    }
    ...
    const std::string command = "open -R \"" + std::string(dir.cstr()) + "\"";
    std::system(command.c_str());
#else
    const std::string command = "xdg-open \"" + std::string(dir.cstr()) + "\"";
    std::system(command.c_str());
#endif
}

void AssetWindow::CreateNewAsset(const char*           assetKind,
...
void AssetWindow::DrawNewAssetMenuItems(const AssetDirectory* targetDirectory) {
...
void AssetWindow::DrawContentBackgroundContextMenu() {
...
void AssetWindow::DrawAsset(const AssetInfo&      asset,
    ...
    if (bgfx::isValid(asset.thumbnail)) {
        ImGui::Image(Syngine::UI::Debug::SImGui::ToImGui(asset.thumbnail),
    ...
    const std::string contextMenuId = "AssetContextMenu##" + asset.displayName;
    if (ImGui::BeginPopupContextItem(contextMenuId.c_str())) {
        if (asset.isDirectory) {
            if (ImGui::MenuItem(IsFavorited(targetDir->relativePath)
                                    ? "Remove from Favorites"
                                    : "Add to Favorites")) {
                ToggleFavorite(targetDir->relativePath, targetDir->name, true);
            }
            if (ImGui::MenuItem("Delete")) {
                RequestDeleteConfirmation(
                    GetDirectoryAbsolutePath(targetDir), true, targetDir->name);
            }
            if (ImGui::MenuItem("Show in File Browser")) {
            ...
            if (ImGui::MenuItem(IsFavorited(asset.path.diskPath)
                                    ? "Remove from Favorites"
                                    : "Add to Favorites")) {
                ToggleFavorite(asset.path.diskPath, asset.displayName, false);
            }
            if (ImGui::MenuItem("Delete")) {
                RequestDeleteConfirmation(
...
void AssetWindow::DrawAssetTree(const char* searchText) {
...
void AssetWindow::DrawAssetDirectory(const AssetDirectory* directory,
                                     const char*           searchText) {
    ...
    for (const auto& child : directory->children) {
        AssetInfo folder{ .path        = { .diskPath = child->name },
                          ...
                          .thumbnail   = IconManager::GetIcon("assets/folder"),
                          .isDirectory = true };
...
void AssetWindow::DrawAssetSearchResults(const AssetDirectory* directory,
                                         const char*           searchText,
                                         int&                  assetIndex) {
    const int columns = std::max(1, TILE_FIT_WIDTH);
    for (const auto& child : directory->children) {
...
void AssetWindow::SetCurrentShownDirectory(const AssetDirectory* directory) {
...
AssetWindow::SearchForDirectory(const std::string&    name,
                                const AssetDirectory* startDirectory) {
...
const AssetDirectory*
AssetWindow::FindDirectoryByRelativePath(const std::string&    relativePath,
                                         const AssetDirectory* startDirectory) {
...
const AssetInfo*
AssetWindow::FindAssetByDiskPath(const std::string&    diskPath,
                                 const AssetDirectory* startDirectory) {
...
void AssetWindow::ToggleFavorite(const std::string& relativePath,
...
void AssetWindow::NavigateToFavorite(const FavoriteEntry& favorite) {
